...
export default {
	...
	mounted() {
		let defaultOption = {
			...
			colors: ['#E88684'], //图表配色方案，不传则使用系统默认配置
			rotate: false, //是否横屏展示
			...
			xAxis: {
				...
				itemCount: 12, //X轴可见区域数据数量（即X轴数据密度），配合拖拽滚动使用（即仅在启用enableScroll时有效）
			...
			yAxis: {
				...
				format: val => {
					let defaultSetting = { type: 'number', fixed: 0, name: 'k' };
			...
			legend: {
				...
				padding: 10,
				margin: 0,
				itemGap:2,//各个item之间的间隔，单位px，默认为10，横向布局时为水平间隔，纵向布局时为纵向间隔
				// fontColor:'666666',
		...
		touchstart(e) {
			...
			histogramuCharts[this.canvasId].showToolTip(e, {
				...
				format: function(item, category) {
					let defaultSetting = { type: 'number', fixed: 0, name: '元' };
					let newName;
